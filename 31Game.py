...
def main():
    ...
    width = 1200
    height = 800
    win = GraphWin('31 Game', width, height)

    startButton = Rectangle(Point(width / 2 - 75, 300), Point(width / 2 + 75, 375))
    startButton.setFill('gray')
    startButton.setOutline('black')
    startButton.draw(win)

    startText = Text(startButton.getCenter(), 'Start')
    startText.setSize(20)
    startText.setStyle('bold')
    startText.draw(win)

    RulesButton = Rectangle(Point(width / 2 - 75, 400), Point(width / 2 + 75, 475))
    RulesButton.setFill('gray')
    RulesButton.setOutline('black')
    RulesButton.draw(win)

    RulesText = Text(RulesButton.getCenter(), 'Rules')
    RulesText.setSize(20)
    RulesText.setStyle('bold')
    RulesText.draw(win)

    QuitButton = Rectangle(Point(width / 2 - 75, 500), Point(width / 2 + 75, 575))
    QuitButton.setFill('gray')
    QuitButton.setOutline('black')
    QuitButton.draw(win)

    QuitText = Text(QuitButton.getCenter(), 'Quit')
    QuitText.setSize(20)
    QuitText.setStyle('bold')
    QuitText.draw(win)

    Logo = Image(Point(width/2 + 20, 160), '31Logo.png')
    Logo.draw(win)

    mainMenu = True

    while mainMenu:
        menuClick = win.checkMouse()
        if buttonClicked(menuClick, startButton):
            break
        if buttonClicked(menuClick, RulesButton):
            webbrowser.open('https://bicyclecards.com/how-to-play/thirty-one/')
        if buttonClicked(menuClick, QuitButton):
            quit()
...
def play(width: int, height: int, win: GraphWin):
    ...
    deckText = Text(Point(deck.getAnchor().getX(), deck.getAnchor().getY() + 110), 'Draw')
    deckText.setStyle('bold')
    deckText.draw(win)

    knockSound = sa.WaveObject.from_wave_file('door-knock-3.wav')
    ...
    discardText = Text(Point(discard.getAnchor().getX(), discard.getAnchor().getY() + 110), 'Discard')
    discardText.setStyle('bold')
    discardText.draw(win)
    ...
    while True:
        ...
        if imageClicked(playerClick, deck):
            ...
            decisionDeckImage = Image(Point(width / 2, discard.getAnchor().getY() + 230), drawName + drawSuit + '.png')
        ...
        if computerTurn:
            ...
            if discardaltList1Value > getHandValue(computerDeck) or discardaltList2Value > getHandValue(
                    computerDeck) or discardaltList3Value > getHandValue(computerDeck):
                ...
                if drawaltList1Value > getHandValue(computerDeck) or drawaltList2Value > getHandValue(
                        computerDeck) or drawaltList3Value > getHandValue(computerDeck):
